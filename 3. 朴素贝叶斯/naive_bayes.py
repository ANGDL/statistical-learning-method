class MultinomialNB:
    ...
    def fit(self, X, y):
        ...
        for x_i, y_i in zip(X, y):
            ...
            for j, x_i_j in enumerate(x_i):
                self.sj[j].add(x_i_j)
                self.__counter(self.joint_xy_counter, (x_i_j, y_i))
    ...
    def predict(self, X):
        ...
        prob_list = []
        prob = [-1.0] * len(self.sj)

        sj = 0

        for i, x_i in enumerate(X, 1):
            prob = dict(zip(self.y_counter.keys(), prob))
            for y_k in self.y_counter:
                for j, x_i_j in enumerate(x_i):
                    if self.lambda_ != 0:
                        sj = len(self.sj[j])
                    try:
                        p_xij_yk = (self.joint_xy_counter[(x_i_j, y_k)] + self.lambda_) / \
            ...
            prob_list.append(max(prob, key=prob.get))
